...
(function () {
    ...
    d.AxisBase = d.Class({construct: function (a) {
        ...
        var a, b = this.chart, c = this.showFirstLabel, e = this.showLastLabel, h, f = "", g = d.extractPeriod(this.minPeriod), k = d.getPeriodDuration(g.period, g.count), l, m, n, q, p, r = this.firstDayOfWeek, t = this.boldPeriodBeginning;
        ...
        var B = Math.ceil(this.timeDifference / A) + 5, D = l = d.resetDateToMin(new Date(this.startTime - A), v, x, r).getTime();
...
(function () {
    ...
    d.RecItem = d.Class({construct: function (a, b, c, e, h, f, g, k, l, m, n, q) {
        ...
        this.set = w;
        var x = a.axisThickness, v = a.axisColor, A = a.axisAlpha, B = a.tickLength, D = a.gridAlpha, C = a.gridThickness, I = a.gridColor, H = a.dashLength, O = a.fillColor, J = a.fillAlpha, da = a.labelsEnabled;
        n = a.labelRotation;
    ...
    d.AmChart = d.Class({construct: function (a) {
        ...
        inIframe: function () {
            ...
            if ("amcharts.com" != b || !0 === this.inIframe()) {
                ...
                var g = "http://www.amcharts.com/javascript-charts/", k = "JavaScript charts", l = "";
...
(function () {
    ...
    d.AmGraph = d.Class({construct: function (a) {
        ...
        isNaN(k) && (k = .8);
        var m = this.useNegativeColorIfDown, n = this.width, q = this.height, p = this.y, r = this.rotate, t = this.columnCount, u = d.toCoordinate(this.cornerRadiusTop, k / 2), z = this.connect, w = [], x = [], v, A, B, D, C = this.chart.graphs.length, I, H = this.dx / this.tcc, O = this.dy / this.tcc, J = f.stackType, da = this.start, pa = this.end, U = this.scrollbar, na = "graph-column-";
        U && (na = "scrollbar-graph-column-");
        var qa = this.categoryAxis, ja = this.baseCoord, Pa = this.negativeBase,
            aa = this.columnIndex, Z = this.lineThickness, V = this.lineAlpha, sa = this.lineColorR, ba = this.dashLength, ca = this.set, ta, ia = this.getGradRotation(), Q = this.chart.columnSpacing, W = qa.cellWidth, wa = (W * k - t) / t;
        Q > wa && (Q = wa);
        var G, y, ka, ea = q + 1, Qa = n + 1, Y = 0, qb = 0, rb, sb, eb, fb, tb = this.fillColorsR, Ja = this.negativeFillColors, Ca = this.negativeLineColor, Wa = this.fillAlphas, Xa = this.negativeFillAlphas;
        ...
        for (K = da; K <= pa; K++) {
            ...
            var T, P, M,
                fa, la = NaN, F = NaN, E = NaN, N = NaN, L = NaN, Na = NaN, Da = NaN, Oa = NaN, Ea = NaN, ya = NaN, za = NaN, ga = NaN, ha = NaN, X = NaN, vb = NaN, wb = NaN, ma = NaN, oa = void 0, Ia = tb, Sa = Wa, Aa = sa, ua, xa, xb = this.proCandlesticks, kb = this.topRadius, $a = this.pattern;
                    ...
                    case "candlestick":
                        if (!isNaN(ma) && !isNaN(X)) {
                            ...
                            if (r) {
                                ...
                                xb && X >= ma && (Sa = 0);
                                var oa = new d.Cuboid(h, M, fa, H, O, Ia, Sa, Z, ua, V, ia, u, r, ba, $a, kb, na), Bb, Cb;
                                X > ma ? (Bb = [E, Ea], Cb = [L,
                                    Da]) : (Bb = [L, Ea], Cb = [E, Da]);
        ...
        animateColumns: function (a, b) {
            ...
            if (1 < a.length) {
                ...
                h.push(l);
                h.push(k);
                var m = this.lineAlpha, n = this.lineThickness, h = this.dashLength, q = this.fillAlphas, p = this.lineColorR, r = this.fillColorsR, t = this.negativeLineColor, u = this.negativeFillColors, z = this.negativeFillAlphas, w = this.baseCoord, x = this.lineColorSwitched;
                x && (p = x);
    ...
    d.ChartCursor = d.Class({construct: function (a) {
        ...
        destroy: function () {
            ...
            this.leaveCursor || (this.hideObj(this.set), this.hideObj(this.categoryBalloon), this.hideObj(this.line), this.hideObj(this.vLine), this.hideObj(this.hLine), this.hideObj(this.vaBalloon), this.hideObj(this.allBullets), this.destroyValueBalloons(), this.selectWithoutZooming || d.remove(this.selection),
                this.previousIndex = NaN, a && this.fire("onHideCursor", {type: "onHideCursor", chart: this.chart, target: this}), this.drawing || this.chart.setMouseCursor("auto"), this.normalizeBulletSize())
            ...
            if (this.enabled) {
                ...
                var g = c.categoryAxis, k = c.dx, l = c.dy, m = this.x + 1, n = this.y + 1, q = this.width, p = this.height, r = this.data[a];
                this.fireMoved();
                if (r) {
                    ...
                    if (c.mouseIsOver || B || D || this.forceShow)if (this.forceShow = !1, D) {
                        var k = this.panClickPos, c = this.panClickEndTime, B = this.panClickStartTime, I = this.panClickEnd, m = this.panClickStart, h = (u ? k - f : k - h) / z;
                        if (!g.parseDates || g.equalSpacing)h = Math.round(h);
                        0 !== h && (k = {type: "zoomed", target: this}, k.chart = this.chart, g.parseDates && !g.equalSpacing ? (c + h > v && (h = v - c), B + h < x && (h = x - B), k.start = Math.round(B + h), k.end = Math.round(c + h), this.fire(k.type, k)) :
            ...
            if (a) {
                ...
                if (this.drawingNow) {
                    ...
                    if (2 < Math.abs(c - e) || 2 < Math.abs(f -
                        h))c = {type: "draw", target: this, chart: a, initialX: c, initialY: f, finalX: e, finalY: h}, this.fire(c.type, c)
                }
    ...
    d.AmBalloon = d.Class({construct: function (a) {
        ...
        if (!isNaN(a)) {
            ...
            if (this.show) {
                ...
                var t = this.horizontalPadding, u = this.verticalPadding, z = this.pointerWidth, w = this.pointerOrientation, x = this.cornerRadius, v = c.fontFamily, A = this.fontSize;
                ...
                D < g && (D = g);
                D + C > l && (D = l - C);
                var k = A + u, m = D + t, u = this.shadowAlpha, H = this.shadowColor, t = this.borderThickness, O = this.bulletSize, J;
...
(function () {
    ...
    d.AmSlicedChart = d.Class({inherits: d.AmChart, construct: function (a) {
        ...
        !0 === c && (e = 0);
        (c = a.wedge) && (0 < e ? c.animate({translate: b * a.pullX +
            "," + b * a.pullY}, e, this.pullOutEffect) : c.translate(b * a.pullX, b * a.pullY));
...
(function () {
    ...
    d.TrendLine = d.Class({construct: function (a) {
        ...
        var a = this;
        a.destroy();
        var b = a.chart, c = b.container, e, h, f, g, k = a.categoryAxis, l = a.initialDate, m = a.initialCategory, n = a.finalDate, q = a.finalCategory, p = a.valueAxis, r = a.valueAxisX, t = a.initialXValue, u = a.finalXValue, z = a.initialValue, w =
...
(function () {
    ...
    d.wedge = function (a, b, c, e, h, f, g, k, l, m, n, q, p) {
        ...
        h >= z &&
        (h = z);
        var w = 1 / 180 * Math.PI, z = b + Math.sin(e * w) * k, x = c - Math.cos(e * w) * t, v = b + Math.sin(e * w) * f, A = c - Math.cos(e * w) * g, B = b + Math.sin((e + h) * w) * f, D = c - Math.cos((e + h) * w) * g, C = b + Math.sin((e + h) * w) * k, w = c - Math.cos((e + h) * w) * t, I = {fill: d.adjustLuminosity(m.fill, -.2), "stroke-opacity": 0, "fill-opacity": m["fill-opacity"]}, H = 0;
        180 < Math.abs(h) && (H = 1);
        e = a.set();
        var O;
...
(function () {
    ...
    d.AmLegend = d.Class({construct: function (a) {
        ...
        var a = this.position, b = this.marginLeft + this.titleWidth, c = this.marginRight, e = this.marginTop, h = this.marginBottom, f = this.horizontalGap, g = this.div, k = this.divWidth, l = this.maxColumns,
